...
namespace webapi.Controllers.Dashboard
{
    ...
    public class DashboardController : ApiController, IDashboardController
    {
        ...
        {
            var Student = _context.TimeTables.Where(ss => ss.User1.userID == UserId).GroupBy(s => s.User.userID);
            ...
            var TutorClasses = _context.TimeTables.Where(c => c.User1.userID == UserId).ToList();
            ...
            var todaysClasses = (from c in TutorClasses
                                 ...
                                 select new
                                 {
                                     ...
                                     lessonName = (from tt in _context.TimeTables 
                                                   join lp in _context.LessonPlans on tt.LessonPlan.lessonPlanID equals lp.lessonPlanID
                                                   where tt.ClassID == c.ClassID
                                                   select lp.lessonName).FirstOrDefault(),
                                    surahName = (from tt in _context.TimeTables
                                                 join lp in _context.LessonPlans on tt.LessonPlan.lessonPlanID equals lp.lessonPlanID
                                                 join l in _context.Lessons on lp.lessonPlanID equals l.LessonPlan.lessonPlanID
                                                 join s in _context.surahs on l.surah.Id equals s.Id
                                                 where tt.ClassID == c.ClassID
                                                 select s.surah_Urdu_Names).FirstOrDefault(),
                                     studentName = c.User.name,
                                     studentProfile = c.User.profile,
                                     studentLocation = c.User.country,
